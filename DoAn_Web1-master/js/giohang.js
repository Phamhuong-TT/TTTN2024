...
function thanhToan() {
    ...
    if (window.confirm('Thanh toán giỏ hàng ?')) {
        ...
        // Tạo mã QR cho đơn hàng mới nhất
...
function generateQRCode(orderData) {
    // Tạo phần tử chứa mã QR nếu chưa tồn tại
    ...
    // Xóa nội dung trước đó nếu có
    qrContainer.innerHTML = '';

    // Dữ liệu để mã hóa trong mã QR
    const qrData = JSON.stringify(orderData); // Mã hóa chi tiết đơn hàng trong mã QR

    // Tạo mã QR
    const qr = new QRious({
        element: qrContainer,
        value: qrData,
        size: 250, // Kích thước của mã QR
        level: 'H' // Mức độ sửa lỗi
    });

    // Tùy chọn: Định dạng container của mã QR
    qrContainer.style.position = 'fixed';
    qrContainer.style.bottom = '60px';
    qrContainer.style.right = '60px';
    qrContainer.style.padding = '100px';
    ...
    titleImage.src = 'img/QR_code.jpg'; // Đường dẫn đến hình ảnh của bạn
    titleImage.alt = 'Mã QR Thanh Toán';
    titleImage.style.width = '200px'; // Kích thước của hình ảnh, có thể điều chỉnh
    titleImage.style.height = 'auto'; // Đảm bảo tỷ lệ kích thước

    // Chèn hình ảnh vào container mã QR
    qrContainer.insertBefore(titleImage, qrContainer.firstChild);
